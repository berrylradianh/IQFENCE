  ...
  void _recordPresensi() {
    ...
    if (_currentPosition == null || _selectedLocationCoord == null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
              'Gagal mendapatkan lokasi saat ini atau lok;EA: lokasi tujuan'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    // Navigasi ke PresensiFotoScreen dengan isDatang: false
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PresensiFotoScreen(
          ...
          isDatang: false, // Menandakan presensi pulang
        ),
      ),
    ).then((result) {
      if (result != null) {
        // Tangani hasil dari PresensiFotoScreen
        final formattedTime = result['pulangTime'];
        final locationName = result['locationName'];
        debugPrint('Presensi dicatat: $locationName pada $formattedTime');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Presensi Pulang berhasil dicatat di $locationName'),
